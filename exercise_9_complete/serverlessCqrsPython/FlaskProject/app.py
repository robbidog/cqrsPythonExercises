...
def remove_reservation():
    try:
        bus = Bus(COMMAND_HANDLERS, EVENT_HANDLERS, QUERY_HANDLERS, contextRepository)
        cmd = Commands.RemoveReservation(request.json)
        bus.handle(cmd)
        return f'OK. Reservation on room of type {cmd.RoomType} has been removed'
    except Exception:
        e = sys.exc_info()
        return 'Error occurred when removing a room reservation. ' + str(e[1]), 400
